-- Defense-in-depth: Supabase Row Level Security (RLS) for `evidence`.
--
-- `evidence` is a CHILD of `activity_records`, which is itself a child of
-- `subsidiaries`. So a row is selectable only if its parent activity record's
-- subsidiary is selectable (explicit user_subsidiary_access row, OR a
-- super_admin / consultant / executive_viewer in the same organisation). This
-- mirrors the `activity_records` policy in 20260701214626_rls_activity_records,
-- reached via a join through the parent record.
--
...
-- ---------------------------------------------------------------------------
-- evidence: selectable if the parent activity record's subsidiary is selectable
-- (same access/role rule as `activity_records`, reached via a two-level join).
-- ---------------------------------------------------------------------------
CREATE POLICY "evidence_select_scoped"
  ...
  USING (
    EXISTS (
      ...
      JOIN "subsidiaries" s ON s."id" = ar."subsidiary_id"
      WHERE ar."id" = "evidence"."activity_record_id"
        AND (
          EXISTS (
            SELECT 1
