-- Defense-in-depth: Supabase Row Level Security (RLS) for `period_locks`.
--
-- `period_locks` is a CHILD of `subsidiaries`, so it is tenant-scoped by
...
-- `locations` policy in 20260630204830_rls_policies.
--
...
-- cannot read cross-tenant lock state.
--
...
-- ---------------------------------------------------------------------------
-- period_locks: selectable if their parent subsidiary is selectable.
-- ---------------------------------------------------------------------------
CREATE POLICY "period_locks_select_scoped"
  ON "period_locks"
  FOR SELECT
  TO "authenticated"
  USING (
    EXISTS (
      SELECT 1
      FROM "subsidiaries" s
      WHERE s."id" = "period_locks"."subsidiary_id"
        AND (
          EXISTS (
            SELECT 1
