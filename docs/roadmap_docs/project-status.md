...
- **Phase:** Phase 1 — Core MVP (Scope 1 & 2) — WP1–WP4 done; **WP5–WP6 remain** (Targets & intensity → Reports; kept in Phase 1 per the 2026-07-15 decision)
- **Latest merged:** PR #17 (WP3 period locking); WP4 E2E & hardening on `feat/e2e-hardening` (uncommitted)
- **Tests:** 118 unit (Vitest, API) + 10 E2E (Playwright) + live RLS containment probes — green
...
- Pending Phase 1 work packages: **WP5** Targets & intensity · **WP6** Reports
...
**Next up:** WP5 — Targets & intensity: replace the `/emissions` "not yet available" state with a real targets + intensity backend (tenant-scoped, `super_admin`-managed, audited), wired to the Targets tab, with E2E via the `e2e-flow` skill. Then WP6 (Reports); the Phase-1 gate lands after WP6.
