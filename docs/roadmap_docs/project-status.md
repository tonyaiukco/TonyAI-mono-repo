...
- **Phase:** Phase 1 — Core MVP (Scope 1 & 2) — WP1–WP5 done; **only WP6 (Reports) remains**
- **Latest merged:** PR #19 (WP4) + #20 (build warnings); WP5 Targets & intensity on `feat/targets-intensity` (uncommitted)
- **Tests:** 139 unit (Vitest, API) + 12 E2E (Playwright) + live RLS containment probes (6 tables) — green
...
**Next up:** WP6 — Reports (the final Phase-1 package): audit-ready PDF (Puppeteer) + Excel/CSV export, filter-aware (FR §5), replacing the Reports-page mock; tenant-scoped generation off the live aggregation. The Phase-1 exit gate lands after WP6.
